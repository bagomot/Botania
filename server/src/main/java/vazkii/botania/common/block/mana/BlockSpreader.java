...
public class BlockSpreader extends BlockMod implements IWandable, IWandHUD, ILexiconable, IWireframeAABBProvider
{
	...
	public BlockSpreader()
	{
		...
		this.setDefaultState(this.blockState.getBaseState().withProperty(BotaniaStateProps.SPREADER_VARIANT, SpreaderVariant.MANA));
	}
	...
	public IBlockState getStateFromMeta(int meta)
	{
		if (meta > SpreaderVariant.values().length)
			meta = 0;
		return this.getDefaultState().withProperty(BotaniaStateProps.SPREADER_VARIANT, SpreaderVariant.values()[meta]);
	}
	...
	public void onBlockPlacedBy(World world, BlockPos pos, IBlockState state, EntityLivingBase placer, ItemStack stack)
	{
		...
		world.setBlockState(pos, this.getStateFromMeta(stack.getItemDamage()), 1 | 2);
		...
		spreader.fake.setProfile(placer);
	...
	{
		return this.getMetaFromState(par1);
	}
