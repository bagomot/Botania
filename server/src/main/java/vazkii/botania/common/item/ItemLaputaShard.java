			...
			else if (burst.getTicksExisted() == placeTicks)
			{
				...
				if (entity.world.isAirBlock(pos))
				{
					...
					if (lens.hasTagCompound())
						...
						else // Attempt to read legacy tag (integer ID) if string block ID is absent
							if (lens.getTagCompound().hasKey(TAG_BLOCK))
					...
					entity.world.setBlockState(pos, state, 1 | 2);
