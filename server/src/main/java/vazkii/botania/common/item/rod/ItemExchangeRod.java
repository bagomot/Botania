...
public class ItemExchangeRod extends ItemMod implements IManaUsingItem, IWireframeCoordinateListProvider
{
	...
	public EnumActionResult onItemUse(EntityPlayer player, World world, BlockPos pos, EnumHand hand, EnumFacing side, float par8, float par9, float par10)
	{
		...
		if (player.isSneaking())
		{
			...
			if (tile == null)
			{
				if (BlockCamo.isValidBlock(wstate))
				{
					...
					this.setBlock(stack, wstate.getBlock(), !item.getHasSubtypes() ? 0 : wstate.getBlock().getMetaFromState(wstate));
		...
		else if (this.canExchange(stack) && !ItemNBTHelper.getBoolean(stack, TAG_SWAPPING, false))
		{
			Block block = getBlock(stack);
			int meta = getBlockMeta(stack);
			List<BlockPos> swap = this.getBlocksToSwap(world, stack, block.getStateFromMeta(meta), pos, null);
			if (swap.size() > 0)
			{
				...
				this.setTargetBlock(stack, wstate.getBlock(), wstate.getBlock().getMetaFromState(wstate));
				if (world.isRemote)
	...
	public void onLeftClick(PlayerInteractEvent.LeftClickBlock event)
	{
		...
		if (!stack.isEmpty() && stack.getItem() == this && this.canExchange(stack) && ManaItemHandler.requestManaExactForTool(stack, event.getEntityPlayer(), COST, false))
		{
			if (this.exchange(event.getWorld(), event.getEntityPlayer(), event.getPos(), stack, getBlock(stack).getStateFromMeta(getBlockMeta(stack))))
	...
	public void onUpdate(ItemStack stack, World world, Entity entity, int something, boolean somethingelse)
	{
		...
		if (ItemNBTHelper.getBoolean(stack, TAG_SWAPPING, false))
		{
			...
			List<BlockPos> swap = this.getBlocksToSwap(world, stack, block.getStateFromMeta(meta), new BlockPos(x, y, z), targetBlock.getStateFromMeta(targetMeta));
			if (swap.size() == 0)
			{
				ItemNBTHelper.setBoolean(stack, TAG_SWAPPING, false);
				return;
			...
			boolean exchange = this.exchange(world, player, coords, stack, block.getStateFromMeta(meta));
			if (exchange)
	...
	{
		return Block.getBlockFromName(getTargetBlockName(stack));
	}
	...
	public List<BlockPos> getWireframesToDraw(EntityPlayer player, ItemStack stack)
	{
		...
		if (pos != null && pos.getBlockPos() != null)
		{
			...
			{
				List<BlockPos> coordsList = this.getBlocksToSwap(player.world, stack, block.getStateFromMeta(meta), bPos, targetBlock == null ? null : targetBlock.getStateFromMeta(targetMeta));
