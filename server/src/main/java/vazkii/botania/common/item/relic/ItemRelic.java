...
public class ItemRelic extends ItemMod implements IRelic
{
	...
	public void addBindInfo(List<String> list, ItemStack stack)
	{
		if (GuiScreen.isShiftKeyDown())
		{
			...
			else if (!this.getSoulbindUUID(stack).equals(Minecraft.getMinecraft().player.getUniqueID()))
	...
	public void updateRelic(ItemStack stack, EntityPlayer player)
	{
		...
		if (!this.hasUUID(stack))
		{
			this.bindToUUID(player.getUniqueID(), stack);
			if (player instanceof EntityPlayerMP)
				RelicBindTrigger.INSTANCE.trigger((EntityPlayerMP) player, stack);
		}
		else if (!this.getSoulbindUUID(stack).equals(player.getUniqueID()))
			rightPlayer = false;
	...
	{
		return this.hasUUID(stack) && this.getSoulbindUUID(stack).equals(player.getUniqueID());
	}
