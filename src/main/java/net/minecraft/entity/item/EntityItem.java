...
public class EntityItem extends Entity
{
    ...
    {
        if (this.getItem().getItem().onEntityItemUpdate(this))
            return;
    ...
    public void onCollideWithPlayer(EntityPlayer entityIn)
    {
        if (!this.world.isRemote)
        {
            ...
            if (this.pickupDelay <= 0 && (this.owner == null || this.lifespan - this.age <= 200 || this.owner.equals(entityIn.getName())) && (hook == 1 || i <= 0 || entityIn.inventory.addItemStackToInventory(itemstack) || clone.getCount() > this.getItem().getCount()))
            {
